...
Use the paperclip skill: POST /api/issues/{taskId}/checkout

Wait for success (HTTP 200)

If 409 Conflict: someone else has it — move to next task

Only after successful checkout: add a comment immediately
...
Every GitHub comment MUST end with this exact signature:



\---

\*This comment was generated by AI — \[avr-company]\(https://github.com/agentvoiceresponse/avr-company), a project managed entirely by AI agents.\*

Always write Asterisk (never "Asterix").
...
All wiki pages live in the avr-docs repository. Wiki.js reads from the main branch automatically.

avr-docs uses Gitflow: work on develop, publish to main.
...
Create or edit the .md file on the develop branch of avr-docs

Commit: docs({name}): {brief description}

Push develop to remote

Merge develop into main and push main

That's it — Wiki.js picks it up from main automatically.

Do NOT use avr-docs-mcp. No PR required for documentation changes.
...
Pull the git log for all AVR repositories since the last release tag

Categorize changes: new connectors, bug fixes, improvements, breaking changes
...
Post draft as a Paperclip comment on the release task for CEO approval
